...
function ItemAvisoDestaque({ aviso }: { aviso: AvisoApp }) {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2 mb-1">
          <h3 className="font-semibold text-foreground">{aviso.title}</h3>
          {config.badge && (
        ...
        <p className="text-sm text-muted-foreground">{aviso.content}</p>
      </div>
    </div>
  );
}
    ...
    <Card className="border-gold/40 bg-gradient-to-br from-gold/10 via-background to-background shadow-md">
      <CardContent className="p-4 sm:p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-lg font-bold text-foreground">
                {lista.length === 1 ? "1 aviso em exibição" : `${lista.length} avisos em exibição`}
              </h2>
            </div>
          </div>
        ...
        <div className="space-y-3">
          {lista.map((aviso) => (
            <ItemAvisoDestaque key={aviso.id} aviso={aviso} />
          ))}
        </div>
      </CardContent>
    </Card>
